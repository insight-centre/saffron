...
public class TaxonomyConstructor {
    ...
    public static Taxonomy optimisedSimilarityGraph(List<DocumentTopic> docTopics,
        ...
        Map<Topic, Map<String, Integer>> occMap = getTopicMap(docTopics, topics);

        int totalDocs = getTotalDocs(docTopics);

        DirectedGraph<Topic> tmpGraph = constructEdgesWithoutDirections(occMap, nodes, simThreshold,
                minCommonDocs);
        ...
        if (tmpGraph.getEdges().size() > 0) {
            ...
            //} else {
            Map<Topic, Double> sumPMIMap = computeSumPMI(occMap, topics, organisedEdges, tmpGraph, totalDocs);
            //}
            System.err.printf("PMI for %d nodes\n", sumPMIMap.size());

            System.err.println("Construct directed edges based on SumPMI");
            DirectedGraph<Topic> edges = constructDirectedEdges(simThreshold, tmpGraph, sumPMIMap);
            System.err.printf("Found %d edges\n", edges.getEdges().size());

            edges = normaliseEdgeWeights(edges);

            System.err.println("Prune the graph");
            DirectedGraph<Topic> prunedEdges = GraphPruning.pruneGraph(edges, sumPMIMap);

            System.err.printf("%d edges remain\n", prunedEdges.getEdges().size());
            ...
            if(prunedEdges.getEdges().isEmpty()) {
    ...
    public static DirectedGraph<Topic> normaliseEdgeWeights(DirectedGraph<Topic> edges) {

        DirectedGraph<Topic> normEdges = new DirectedGraph<>(edges.getNodes());
        ...
        for (Edge edge : edges.getEdges()) {
            ...
            normEdges.addEdge(edges.getNode(edge.getFrom()),
                    edges.getNode(edge.getTo()), intWeight);
//                (new Edge(edge.getFrom(), edge.getTo(), intWeight));
        }

        return normEdges;
    }

    public static DirectedGraph<Topic> constructEdgesWithoutDirections(
            Map<Topic, Map<String, Integer>> occMaps, List<Topic> nodes,
            ...
            for (int j = 0; j < nodes.size(); j++) {
                ...
                if ((nodes.get(i) != nodes.get(j)) && (i >= j)) {
                    ...
                    if (topic1ID.endsWith(" " + topic2ID) || topic2ID.endsWith(" " + topic1ID)
                        ...
                        result.addEdge(nodes.get(i), nodes.get(j), 0.5);
                    } else {
                        ...
                        // them
                        // together
                        int cooc = computeCoocurrence(occMap1, occMap2);
                        if (cooc >= minCommonDocs) {
                            ...
                            if (weight >= simThreshold) {
                                ...
                                result.addEdge(nodes.get(i), nodes.get(j), weight);
                            }
                        }
                    }
                }
    ...
    private static Map<Topic, Double> computeSumPMI(Map<Topic, Map<String, Integer>> sd, Map<String, Topic> topics,
            Map<String, List<Edge>> organisedEdges, DirectedGraph graph, int totalDocs) throws SearchException {
    ...
    public static double sumPMI(Map<Topic, Map<String, Integer>> sd, Topic topic, 
            Map<String, List<Edge>> organisedEdges, DirectedGraph<Topic> graph, int totalDocs) throws SearchException  {
        ...
        if (edges != null) {
            for (Edge edge : edges) {
                ...
                if (sd.containsKey(node1) && sd.containsKey(node2)) {
                    double pxy = (double)computeCoocurrence(sd.get(node1), sd.get(node2)) / totalDocs;
                    double px = (double)sd.get(node1).size() / totalDocs;
                    double py = (double)sd.get(node2).size() / totalDocs;

                    if(pxy > 0) 
                        contextWordsRank += Math.log(pxy / (px * py));
                } else {
                    System.err.printf("Lost %s <-> %s\n", node1, node2);
                }
            }
        }
    ...
    private static DirectedGraph<Topic> constructDirectedEdges(double simThreshold, 
            DirectedGraph<Topic> tmpGraph, Map<Topic, Double> sumPMIMap) {
        DirectedGraph<Topic> graph = new DirectedGraph<>(tmpGraph.nodes);
        for (Edge tmpEdge : tmpGraph.getEdges()) {

            final double weight = tmpEdge.getWeight();
            Topic node1 = tmpGraph.getNode(tmpEdge.getFrom());
            Topic node2 = tmpGraph.getNode(tmpEdge.getTo());
            if(!(node1.equals(node2)) && weight > simThreshold) {
                graph = addEdge(sumPMIMap, graph, tmpEdge.getWeight(), node1, node2);
            }
        }
        return graph;
    }
