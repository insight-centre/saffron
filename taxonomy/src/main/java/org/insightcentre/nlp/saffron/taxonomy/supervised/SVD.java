...
public abstract class SVD {
    ...
    public Matrix solve() {
        if(vectors.isEmpty()) {
            throw new IllegalStateException("Solving on empty matrix");
        } else if(vectors.size() + 1 < vectors.get(0).length) {
            throw new IllegalStateException("Insufficient number of examples to solve");
        }
