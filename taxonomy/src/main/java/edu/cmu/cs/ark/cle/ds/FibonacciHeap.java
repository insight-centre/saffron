...
public class FibonacciHeap<V,P> implements Iterable<FibonacciHeap<V,P>.Entry> {
	...
	public Iterator<Entry> iterator() {
            ...
            _siblingsAndBelow(oMinEntry, values);
            return values.iterator();
		//return siblingsAndBelow(oMinEntry);
	}

        private void _siblingsAndBelow(Optional<Entry> oEntry, ArrayList<Entry> entries) {
            if(!oEntry.isPresent()) return;
            for(Entry e : getCycle(oEntry.get())) {
                entries.add(e);
                _siblingsAndBelow(e.oFirstChild, entries);
            }
        }
