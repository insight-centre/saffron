...
//    // ClefCiting 0.0, 10, 2
//    // Toine 0.0. 5, 3 
//    constructGraphWithTopDBTopics(db, top, 0.0, 5, 3, false);
//    // subsumptionGraph(top, 0.05);
//  }
//
//  private static void constructGraphWithTopDBTopics(DAO db, Integer top,
...
//	 
//	List<String> topics = db.topRankingTopicStrings(top);
//    TaxonomyConstructor.optimisedSimilarityGraph(db, null, new File("./"), 
...
// * @throws IOException 
//   */
//  private static void similarityGraph(DAO db, DocumentSearcher sd, Integer nodesNumber, Double simThreshold,
...
//          Subsumption.computeSumPMI(db, sd, db.topRankingTopicStrings(nodesNumber),
//              useTermsForSumPMI, "pmi.txt");
//      // readSumPMIMap();
//      logger.log(Level.INFO, "Construct edges based on similarity");
//      TaxonomyConstructor tc = new TaxonomyConstructor();
//      List<Node> nodes =
//          tc.convertTopicsToNodes(db.topRankingTopicStrings(nodesNumber));
//      // List<Node> nodes =
//      // TaxonomyConstructor.convertTopicsToNodes(new ArrayList<String>(
//      // sumPMIMap.keySet()));
//
//      AdjacencyList edges =
//          constructSimEdges(db, sd, nodes, sumPMIMap, simThreshold, simMethod,
//              minCommonDocs);
//
//      EMReportManager emrm = new EMReportManager();
//      edges = emrm.normaliseEdgeWeights(edges);
//
//      JGraphTRepresentation.exportToDot(db,
...
// * @throws IOException 
//   */
//  private static void subsumptionGraph(DAO db, DocumentSearcher sd, Integer nodesNumber, Double simThreshold) throws SearchException, IOException, SQLException {
//
//    List<String> topics = new ArrayList<String>();
//      topics = db.topRankingTopicStrings(nodesNumber);
//
//      logger.log(Level.INFO, "Constructing the nodes..");
//      TaxonomyConstructor tc = new TaxonomyConstructor();
...
//          Subsumption.computeSumPMI(db, sd, topics, true, "pmi.txt");
//
//      logger.log(Level.INFO, "Constructing edges based on subsumption..");
//      // AdjacencyList edges =
//      // constructSubsumpSimEdges(nodes, pmiMap, simThreshold);
//      AdjacencyList edges = constructSubsumpEdges(db, sd, nodes, pmiMap);
//
//      JGraphTRepresentation.exportToDot(db,
//          JGraphTRepresentation.convertToJGraphT(nodes, edges),
...
//  }
//
//  private static AdjacencyList constructSimEdgesOptimised(DAO db, List<Node> nodes,
...
//      Topic t1 = db.getTopic(nodei.getTopicString());
//      for (Node nodej : nodes) {
//        Topic t2 = db.getTopic(nodej.getTopicString());
//        if (nodei != nodej) {
//
//          weight =
...
//  }
//
//  private static AdjacencyList constructSimEdges(DAO db, DocumentSearcher sd, List<Node> nodes,
...
//            // them together
//            if (db.selectCountJointTopics(t1.getRootSequence(),
...
//            TopicSimilarity ts =
//                (TopicSimilarity) db.getTopicSimilarity(nodei.getTopicString(),
...
//  }
//
//  private static AdjacencyList constructSubsumpEdges(DAO db, DocumentSearcher sd, List<Node> nodes,
...
//                .getTopicString()));
//
//        if (!t1.equals(t2) && Subsumption.subsumes(db, sd, t1, t2, docsCount)) {
//
...
//  }
//
//  private static AdjacencyList constructSubsumpSimEdges(DAO db, DocumentSearcher sd, List<Node> nodes,
...
//                      .getTopicString())));
//
//          if (Subsumption.subsumes(db, sd, t1, t2, docsCount)) {
//            edges.addEdge(nodei, nodej, weight);
//          }
//        }
//      }
