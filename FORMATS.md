...
This document describes the formats used by Saffron. All formats are in Json
and we will describe only the properties each has

# Saffron - Formats

This document describes the formats used and generated by Saffron.
...
## Input formats
### Corpus  (e.g. examples/presidential_speech_dataset/corpus_inline_with_authors.json)
...
* `documents`: A list of documents in the corpus. Each document of the list should have *at least* one of the following four:
    * `file`: A string referring to the original version of this document on disk (absolute or relative path) (eg. examples/presidential_speech_dataset/corpus_with_authors.json).
    * `id`: A unique string to identify the document
    * `url`: The URL of the file
    * `contents`:  The text contents of the file (eg. examples/presidential_speech_dataset/corpus_inline_with_authors.json)
...
### Config (eg. examples/config.json)
...
See the [wiki](https://github.com/insight-centre/saffron/wiki/5.-Saffron-Approaches) for the details on what each property is for and the scientific explaination of the method. The config file contains each of the Saffron steps:
...
* `termExtraction`: This element contains the following properties to set up:
    ...
    * `stopWords`: The path to the list of stop words (one per line) if different from the [default stopwords file](see /taxonomy/src/main/resources/stopwords/README).
...
* `taxonomy`:  Taxonomy extraction with Pairwise Scoring model training. It contains the following properties to set up:
    ...
    * `features` : The features to use, each of them can be set to "false" or "true", with default to "false" (see example below). The default for the overall "features" property is set to null.
        ...
        * `featureSelection`:  The feature selection (or null for all features). Each of them can be set to "false" or "true", with default to "false". The default for the overall "featureSelection" property is set to null. For more information on the implementation of these features see taxonomy/src/main/java/org/insightcentre/nlp/saffron/taxonomy/supervised/Features.java ***Command Line Interface Only***. Choose between the following features:
            ...
            * `topicDiff`: uses the Topic Difference feature: document topic complement difference which is defined as $`\mid A \cap  B \mid / \mid A \mid - \mid A \cap B \mid / \mid B \mid`$
...
### Terms (eg. examples/presidential_speech_saffron_output/terms.json)
...
* `status`: Whether the term was validated or not (see the [Review mode](https://github.com/insight-centre/saffron/wiki/2.1.1.-Review-mode-(discontinued)) documentation). The default is set to **none**
* `mv_list`: A list of alternative (morphological variants) forms of this
term string
   * `string`: The form of this variant


### Doc-Terms (eg. examples/presidential_speech_saffron_output/doc-terms.json)
...
### Term-Sim (eg. examples/presidential_speech_saffron_output/term-sim.json)

This file gathers and compares all pair of terms extracted in the previous stage. Each element describes one edge, ie. a relation between two terms, and their similarity score (see the [pairwise scoring](https://github.com/insight-centre/saffron/wiki/5.3.-Taxonomy-Construction#321-pairwise-scoring) step for more explanation on how this is calculated).
...
### Taxonomy (eg. examples/presidential_speech_saffron_output/taxonomy.json))
...
### Author-Terms (eg. examples/presidential_speech_saffron_output/author-terms.json))
...
### Author-Sim (eg. examples/presidential_speech_saffron_output/author-sim.json))
